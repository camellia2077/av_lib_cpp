...
# 递归扫描 + 指定输出根目录
python -m apps.av_tools.run move-by-actor --input "D:\\videos" --recursive --output "D:\\actors"

# 禁用环境代理
python -m apps.av_tools.run move-by-actor --input "D:\\videos" --no-env-proxy

# 当 API 未启动时自动拉起（可覆盖 runtime.toml）
python -m apps.av_tools.run move-by-actor --input "D:\\videos" --auto-start-api

# 若本次自动拉起了 API，任务结束后自动关闭它
python -m apps.av_tools.run move-by-actor --input "D:\\videos" --auto-start-api --stop-api-on-exit
...
默认路径：`apps/av_tools/move_by_actor/runtime.toml`
