...
#include "data/fast_query_db.hpp"
#include <filesystem>
...
void database_manager::ensure_data_directory_exists() {
...
std::string
database_manager::get_db_filepath(const std::string &db_name) const {
  return data_directory_path_ + "/" + db_name;
}

void database_manager::load_default_database() {
  std::string full_path = get_db_filepath(current_db_name_);
  dbs_[current_db_name_] = std::make_unique<FastQueryDB>(full_path);
}

bool database_manager::create_database(const std::string &db_name_raw) {
...
bool database_manager::switch_to_database(const std::string &db_name) {
...
bool database_manager::database_exists(const std::string &db_name) const {
  // 始终检查文件系统中的完整路径
  return std::filesystem::exists(get_db_filepath(db_name));
}

IIdRepository *database_manager::get_current_db() const {
...
const std::string &database_manager::get_current_db_name() const {
  return current_db_name_;
}

std::vector<std::string> database_manager::get_all_db_names() const {
